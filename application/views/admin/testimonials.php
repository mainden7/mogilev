...
<div id="addTestimonial" class="modal fade" role="dialog">
	<div class="modal-dialog">
		...
		<div class="modal-content">
			...
			<form class="form-horizontal mt25 ml50" action="<?php echo base_url(); ?>admin_panel/testimonials" method="POST"   enctype="multipart/form-data">
							...
							<?php foreach ($data['testimonials'] AS $testimonial) { ?>
								<tr>
									...
									<?php if(isset($testimonial['photo'])) { ?>
										<img src="<?php echo base_url() ?>application/resources/user_resources/img/<?php echo $testimonial['photo'] ?>" data-placement="bottom">
										<?php } ?>
									</td>
									...
									<td class="table-icon-cell">
										<a href="<?php echo base_url(); ?>admin_panel/delete_testimonial/<?php echo $testimonial['id']; ?>" onclick="return window.confirm('Вы уверены?')"><i class="font-icon font-icon-trash"></i></a>
									</td>
								</tr>
								<div id="editTestimonial<?php echo $testimonial['id']; ?>" class="modal fade" role="dialog">
									<div class="modal-dialog">
										...
										<div class="modal-content">
											...
											<form class="form-horizontal mt25 ml50" action="<?php echo base_url(); ?>admin_panel/edit_testimonial" method="POST"   enctype="multipart/form-data">
												<div class="modal-body">
													<div class="form-group">
														<label for="name" class="control-label">Имя</label>
												...
												<div class="modal-footer">
													<input id="" type="hidden" value="<?php echo $testimonial['id'] ?>" name="id">
													<input id="<?php echo $csrf_token_name; ?>" type="hidden" value="<?php echo $csrf_hash; ?>" name="<?php echo $csrf_token_name; ?>">                	
													<button type="button" class="btn btn-rounded btn-default" data-dismiss="modal">Закрыть</button>
													<button type="submit"  class="btn btn-rounded btn-primary">Сохранить</button>
												</div>
