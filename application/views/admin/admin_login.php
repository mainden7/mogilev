...
<?php if(isset($_GET['check'])) { ?>
		<?php if($_GET['check'] == 'confirm') { ?>
			...
			<a href="<?php echo base_url(); ?>index.php/User/user_login">войти</a>
		<?php }elseif($_GET['check'] == 'error'){ ?>
			<p class="error">Такого Email'a не существует</p>
			<a href="<?php echo base_url(); ?>index.php/User/user_login">войти</a>
		<?php } ?>
<?php }else{ ?>
	<form id="form-signup_v1" name="form-signup_v1" method="POST" action='<?php echo base_url(); ?>index.php/Login/admin_login'>
	...
	<!-- <form action="<?php echo base_url(); ?>index.php/User/forgot_pass" method='POST'>
