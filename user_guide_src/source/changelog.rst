...
-  Fixed a bug (#4605) - :doc:`Config Library <libraries/config>` method ``base_url()`` stripped trailing slashes from relative URIs passed to it.
...
-  General Changes
   ...
   -  Changed :doc:`Config Library <libraries/config>` methods ``base_url()``, ``base_url()`` to allow protocol-relative URLs by passing an empty string as the protocol.
...
-  Core
   ...
   -  :doc:`Config Library <libraries/config>` changes include:
      ...
      -  Changed ``base_url()`` method  to accept an array as well.
      ...
      -  Added an optional second parameter to both ``base_url()`` and ``base_url()`` that allows enforcing of a protocol different than the one in the *base_url* configuration setting.
...
-  Fixed a bug (#938) - :doc:`Config Library <libraries/config>` method ``base_url()`` added a question mark to the URL string when query strings are enabled even if it already existed.
-  Fixed a bug (#999) - :doc:`Config Library <libraries/config>` method ``base_url()`` always appended ``$config['url_suffix']`` to the end of the URL string, regardless of whether a query string exists in it.
...
-  Fixed a bug - ``form_open()`` compared $action against ``base_url()`` instead of ``base_url()``.
